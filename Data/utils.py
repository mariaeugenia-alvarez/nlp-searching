...
def load_cinema_reviews(datasets_path, corpus_cine_folder):
    ...
    review_paths = glob.glob(os.path.join(datasets_path, corpus_cine_folder, "*.xml"))

    reviews_dict = dict()
    sorted_review_paths = sorted(
        review_paths, key=lambda x: int(x.split(os.path.sep)[-1].split(".")[0])
    )
...
def load_movie_titles(datasets_path, movie_titles_file):
    ...
    with open(
        os.path.join(datasets_path, movie_titles_file), "r", encoding="utf-8"
    ) as f:
        titles = [movie_tuple(movie) for movie in map(matcher, f) if movie]
    return titles
